...
def plot_cvae_dif_mat(ABIDE_data, z_encoder, s_encoder, df, sub_slice, keys=None, l=5, w=2, ax=None, title=None,
    ...
    dmat = np.array(
        [get_sil_diff(ABIDE_data, z_encoder, s_encoder, df, sub_slice, keys=keys, l=l, w=w) for i in range(10)])
...
def plot_trainProgress(loss, im, im1):
    ...
    if len(loss) > n:
        plt.plot(loss[-n::]);
        plt.title(f'loss: last {n} iteration');
...
def plot_pca_rsa(keys, df, ABIDE_data, patients, encs, xlbls=['BG', 'SL', 'VAE'], thresh=.25):
    ...
    for i in range(n_components):
        rsa_results = [fit_rsa(components[:, i], ABIDE_data[~isnan, :, :, :], patients[~isnan], encs) for _ in
                       range(10)]
